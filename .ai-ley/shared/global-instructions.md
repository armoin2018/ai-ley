...
**For comprehensive testing infrastructure, use the `build-unit-tests` prompt:**

```bash
ai-ley build-unit-tests [--mode=plan|dry-run|apply]
```

See `.ai-ley/shared/prompts/build-unit-tests.md` for complete testing standards and implementation.

### Key Testing Standards Summary

All testing must follow the comprehensive standards defined in the `build-unit-tests` prompt, including:

**Unit Tests**:

- All new functions must have corresponding unit tests
- Maintain ≥80% code coverage (enforced via coverage tools)
...
- Cover interactions between multiple components or systems
...
- Use **transaction-based database tests** with automatic rollback for clean state
...
- **CRUD Testing**: For all database operations, test all four actions (Create, Read, Update, Delete) with sample data validation
- Ensure proper cleanup after tests with rollback mechanisms

**Test Organization**:

- Tests stored in `{{folders.tests}}` directory
- Use descriptive names for test files and directories
- Include clear README.md file in `{{folders.tests}}` directory (per `{{files.tests}}`)
- Group related tests using subdirectories (unit/, integration/, health/, regression/, synthetic/, performance/, security/)
- Relate tests to specific versions and builds via registry.yaml
- Continuously monitor and improve test coverage and quality

**Testing Infrastructure** (generated by `build-unit-tests` prompt):
...
- Test Control API for remote execution
- Coverage dashboards and reporting
- CI/CD integration with timeout/retry mechanisms

For detailed implementation, examples, and tooling setup, execute the `build-unit-tests` prompt.
...
- Produce test scripts to validate data protection measures under `{{folders.tests}}/security/data_protection/` (use `build-unit-tests` prompt for security test infrastructure).
...
- Create profiling test scripts and reports under `{{folders.performance}}` or `{{folders.tests}}/performance/` (use `build-unit-tests` prompt for performance test infrastructure).
