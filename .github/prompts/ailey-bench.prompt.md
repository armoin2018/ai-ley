...
keywords: [benchmark, quality, assessment, personas, instructions, performance, prompt, ailey]
---
...
- If a specific filename is provided, search for it in both `.github/ai-ley/personas` and `.github/ai-ley/instructions` directories
- If "personas" is specified, process all files in `.github/ai-ley/personas/**/*.md`
...
2. **File Resolution**:
   ...
   - For specific files: Search in both `.github/ai-ley/personas/**/*.md` and `.github/ai-ley/instructions/**/*.md`
...
```bash
find .github/ai-ley/personas .github/ai-ley/instructions -name "*.md" -not -name "*README*" -not -name "*template*" -not -name "*example*" -type f -exec md5sum {} \; > {{folders.benchmark}}/all-files.md5
```

For personas only:

```bash
find .github/ai-ley/personas -name "*.md" -not -name "*README*" -not -name "*template*" -not -name "*example*" -type f -exec md5sum {} \; > {{folders.benchmark}}/personas.md5
```

For instructions only:

```bash
find .github/ai-ley/instructions -name "*.md" -not -name "*README*" -not -name "*template*" -not -name "*example*" -type f -exec md5sum {} \; > {{folders.benchmark}}/instructions.md5
```

For incremental updates (compare against previous run):

```bash
diff <(find .github/ai-ley/personas .github/ai-ley/instructions -name "*.md" -not -name "*README*" -not -name "*template*" -not -name "*example*" -type f -exec md5sum {} \; | sort) {{folders.benchmark}}/all-files.md5 | grep '^>' | awk '{print $2}'
```
...
This prompt works best with the **ailey-optimizer** agent from `.github/agents/ailey-optimizer.agent.md`.
...
2. **Benchmark all personas:**
   ...
   ```
   Target: "personas"
   Result: Benchmarks all .md files in .github/ai-ley/personas recursively
   ```
...
3. **Benchmark all instructions:**
   ...
   ```
   Target: "instructions"
   Result: Benchmarks all .md files in .github/ai-ley/instructions recursively
   ```
