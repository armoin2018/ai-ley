...
- **CRITICAL**: Follow all testing requirements including:
...
- `.github/ai-ley/instructions/development/testing/**/*.md`
- `.github/ai-ley/instructions/best-practices/testing/**/*.md`

These provide domain-specific guidance and best practices.

## Recommended Agent

This prompt works best with the **ailey-tester** agent from `.github/agents/ailey-tester.agent.md`.
...
- Coverage collection with quality thresholds (≥80%)
...
**Define test directory structure** (per `{{folders.tests}}`):
...
├── coverage-policy.md        # Coverage thresholds (≥80%)
...
**Generate unit tests for existing code** following testing best practices:
...
**Test template structure** (AAA pattern):
...
class TestUserRepositoryCRUD:
    """Test all CRUD operations with transaction rollback."""
...
**Enforce coverage thresholds and create reporting dashboards** (≥80% requirement):
...
### Step 14: Error Logging and Automatic Resolution

**Implement comprehensive error logging and automatic error resolution during test execution**:

#### Error Logging Infrastructure

Create `{{folders.tests}}/error_handler.py`:

```python
"""
Comprehensive error logging and automatic resolution system for test execution.
Generated by: ai-ley build-unit-tests | timestamp: 2025-11-10T00:00:00Z
"""
import json
import logging
...
ERROR_LOG_DIR = Path("{{folders.tests}}/artifacts/error-logs")
ERROR_LOG_DIR.mkdir(parents=True, exist_ok=True)

RESOLUTION_LOG = ERROR_LOG_DIR / "resolution-history.jsonl"
ERROR_SUMMARY = ERROR_LOG_DIR / "error-summary.json"

# Maximum retry attempts before requesting operator assistance
MAX_AUTO_RESOLVE_ATTEMPTS = 10

@dataclass
class TestError:
    """Structured test error information."""
    error_id: str
    test_name: str
    error_type: str
    error_message: str
    ...
    operator_assistance_requested: bool = False
...
class TestErrorLogger:
    ...
    def __init__(self):
        self.error_registry: Dict[str, TestError] = {}
        self.load_error_history()

        # Configure logging
        self.logger = logging.getLogger("test_error_logger")
        self.logger.setLevel(logging.DEBUG)

        # File handler for detailed logs
        fh = logging.FileHandler(ERROR_LOG_DIR / f"test-errors-{datetime.now().strftime('%Y%m%d')}.log")
    ...
    def load_error_history(self):
        """Load previous error resolutions from history."""
        if RESOLUTION_LOG.exists():
            with open(RESOLUTION_LOG, 'r') as f:
                for line in f:
                    if line.strip():
                        error_data = json.loads(line)
                        error = TestError(**error_data)
                        self.error_registry[error.error_id] = error

    def generate_error_id(self, error_type: str, error_message: str, file_path: str, line_number: int) -> str:
        """Generate unique error ID based on error characteristics."""
        error_signature = f"{error_type}:{error_message}:{file_path}:{line_number}"
    ...
    def log_error(
        ...
        """Log a test error and return error ID."""
        error_id = self.generate_error_id(error_type, error_message, file_path, line_number)

        if error_id in self.error_registry:
            # Existing error - increment resolution attempts
            self.error_registry[error_id].resolution_attempts += 1
            self.error_registry[error_id].timestamp = datetime.utcnow().isoformat()
        else:
            # New error - create entry
            self.error_registry[error_id] = TestError(
                error_id=error_id,
                test_name=test_name,
        ...
        # Log to file
        self.logger.error(
            f"Test Error [{error_id}] in {test_name}: {error_type} - {error_message}\n"
            f"Location: {file_path}:{line_number}\n"
            f"Attempt: {self.error_registry[error_id].resolution_attempts}/{MAX_AUTO_RESOLVE_ATTEMPTS}"
        )

        # Save to resolution log
        self._append_to_resolution_log(self.error_registry[error_id])

        return error_id

    def _append_to_resolution_log(self, error: TestError):
        """Append error to resolution history log."""
        with open(RESOLUTION_LOG, 'a') as f:
            f.write(json.dumps(error.to_dict()) + '\n')

    def mark_resolved(self, error_id: str, resolution_strategy: str):
        """Mark an error as resolved with the strategy used."""
        if error_id in self.error_registry:
            self.error_registry[error_id].resolved = True
            self.error_registry[error_id].resolution_strategy = resolution_strategy
            self.logger.info(f"Error [{error_id}] resolved using strategy: {resolution_strategy}")
            self._append_to_resolution_log(self.error_registry[error_id])

    def should_request_operator_assistance(self, error_id: str) -> bool:
        """Check if error has exceeded max auto-resolution attempts."""
        if error_id in self.error_registry:
            error = self.error_registry[error_id]
            return error.resolution_attempts >= MAX_AUTO_RESOLVE_ATTEMPTS and not error.resolved
        return False

    def request_operator_assistance(self, error_id: str) -> Dict:
        """Request operator assistance for unresolvable error."""
        if error_id not in self.error_registry:
            return {"error": "Unknown error ID"}

        error = self.error_registry[error_id]
        error.operator_assistance_requested = True
        ...
        assistance_request = {
            ...
            "resolution_attempts": error.resolution_attempts,
            "traceback": error.full_traceback,
            "timestamp": datetime.utcnow().isoformat(),
            "message": f"OPERATOR ASSISTANCE REQUIRED: Test '{error.test_name}' failed {error.resolution_attempts} times. Manual intervention needed."
        }

        # Log critical assistance request
        self.logger.critical(json.dumps(assistance_request, indent=2))

        # Save to separate assistance request file
        assistance_file = ERROR_LOG_DIR / "operator-assistance-requests.jsonl"
        with open(assistance_file, 'a') as f:
            f.write(json.dumps(assistance_request) + '\n')

        self._append_to_resolution_log(error)

        return assistance_request

    def generate_error_summary(self) -> Dict:
        """Generate summary of all errors and resolutions."""
        total_errors = len(self.error_registry)
        resolved_errors = sum(1 for e in self.error_registry.values() if e.resolved)
        pending_errors = total_errors - resolved_errors
        assistance_requested = sum(1 for e in self.error_registry.values() if e.operator_assistance_requested)
        ...
        summary = {
            ...
            "operator_assistance_requested": assistance_requested,
            "errors_by_type": {},
            "errors_by_test": {},
        ...
        for error in self.error_registry.values():
            ...
            # Count by test
            summary["errors_by_test"][error.test_name] = \
                summary["errors_by_test"].get(error.test_name, 0) + 1

            # Count resolution strategies
            if error.resolution_strategy:
                summary["resolution_strategies"][error.resolution_strategy] = \
                    summary["resolution_strategies"].get(error.resolution_strategy, 0) + 1
...
error_logger = TestErrorLogger()
```

#### Automatic Error Resolution Engine

Create `{{folders.tests}}/auto_resolver.py`:

```python
"""
Automatic error resolution engine for test failures.
Generated by: ai-ley build-unit-tests | timestamp: 2025-11-10T00:00:00Z
"""
import re
import subprocess
...
from pathlib import Path
from error_handler import error_logger, MAX_AUTO_RESOLVE_ATTEMPTS

class AutoResolver:
    """Automatically resolve common test errors and retry."""

    def __init__(self):
        self.resolution_strategies = {
            "ImportError": self._resolve_import_error,
            "ModuleNotFoundError": self._resolve_module_not_found,
            "AttributeError": self._resolve_attribute_error,
            "AssertionError": self._resolve_assertion_error,
            "TimeoutError": self._resolve_timeout_error,
            "ConnectionError": self._resolve_connection_error,
            "FileNotFoundError": self._resolve_file_not_found,
            "PermissionError": self._resolve_permission_error,
            "TypeError": self._resolve_type_error,
            "ValueError": self._resolve_value_error,
        }
    ...
    def resolve_and_retry(
        ...
        """
        Attempt to resolve error and retry test.

        Returns:
            Tuple of (success, resolution_strategy)
        """
        # Check if we've exceeded max attempts
        if error_logger.should_request_operator_assistance(error_id):
            error_logger.request_operator_assistance(error_id)
            return False, "OPERATOR_ASSISTANCE_REQUESTED"

        # Try to resolve using appropriate strategy
        resolver = self.resolution_strategies.get(error_type, self._generic_resolver)

        try:
            resolution_strategy = resolver(error_message, test_command)

            if resolution_strategy:
                error_logger.logger.info(
                    f"Attempting resolution for error [{error_id}] using strategy: {resolution_strategy}"
                )
                ...
                if result.returncode == 0:
                    # Test passed after resolution
                    error_logger.mark_resolved(error_id, resolution_strategy)
                    return True, resolution_strategy
                else:
                    # Test still failing - log and continue
                    error_logger.logger.warning(
                        f"Resolution strategy '{resolution_strategy}' did not fix error [{error_id}]"
                    )
                    return False, None
            else:
                return False, None
            ...
            error_logger.logger.error(f"Error during auto-resolution: {str(e)}")
            return False, None

    def _resolve_import_error(self, error_message: str, test_command: str) -> Optional[str]:
        """Resolve import errors by installing missing packages."""
        # Extract module name from error message
        match = re.search(r"No module named '(\w+)'", error_message)
        if match:
            module_name = match.group(1)
            error_logger.logger.info(f"Installing missing module: {module_name}")
            ...
            try:
                ...
                return f"pip_install:{module_name}"
            except subprocess.CalledProcessError:
                error_logger.logger.warning(f"Failed to install {module_name}")

        return None

    def _resolve_module_not_found(self, error_message: str, test_command: str) -> Optional[str]:
        """Resolve ModuleNotFoundError."""
        return self._resolve_import_error(error_message, test_command)

    def _resolve_attribute_error(self, error_message: str, test_command: str) -> Optional[str]:
        """Resolve attribute errors by checking for typos or missing imports."""
        # Log for manual review - attribute errors often require code fixes
        error_logger.logger.info("AttributeError detected - may require code fix")
        return None

    def _resolve_assertion_error(self, error_message: str, test_command: str) -> Optional[str]:
        """Resolve assertion errors by analyzing expected vs actual."""
        # Assertion errors typically indicate logic issues - flag for review
        error_logger.logger.info("AssertionError detected - requires test or code review")
        return None

    def _resolve_timeout_error(self, error_message: str, test_command: str) -> Optional[str]:
        """Resolve timeout errors by increasing timeout or optimizing."""
        # Increase timeout in test configuration
        error_logger.logger.info("TimeoutError detected - consider increasing timeout threshold")
        return "timeout_increase"

    def _resolve_connection_error(self, error_message: str, test_command: str) -> Optional[str]:
        """Resolve connection errors by checking service availability."""
        # Check if services are running
        error_logger.logger.info("ConnectionError detected - checking service availability")
        # Could implement service health checks here
        return "service_restart"

    def _resolve_file_not_found(self, error_message: str, test_command: str) -> Optional[str]:
        """Resolve file not found errors by creating missing test fixtures."""
        ...
        if match:
            ...
            if not file_path.parent.exists():
                ...
                error_logger.logger.info(f"Created missing directory: {file_path.parent}")
                return f"create_directory:{file_path.parent}"

        return None

    def _resolve_permission_error(self, error_message: str, test_command: str) -> Optional[str]:
        """Resolve permission errors."""
        error_logger.logger.warning("PermissionError detected - may require manual intervention")
        return None

    def _resolve_type_error(self, error_message: str, test_command: str) -> Optional[str]:
        """Resolve type errors."""
        error_logger.logger.info("TypeError detected - requires code review")
        return None

    def _resolve_value_error(self, error_message: str, test_command: str) -> Optional[str]:
        """Resolve value errors."""
        error_logger.logger.info("ValueError detected - check test data and validation")
        return None

    def _generic_resolver(self, error_message: str, test_command: str) -> Optional[str]:
        """Generic resolution strategy for unknown error types."""
        error_logger.logger.info(f"Unknown error type - logging for manual review")
        return None
...
Create `{{folders.tests}}/conftest.py` (or update existing):

```python
"""
Pytest configuration with automatic error resolution.
Generated by: ai-ley build-unit-tests | timestamp: 2025-11-10T00:00:00Z
"""
import pytest
import sys
...
def pytest_runtest_makereport(item, call):
    ...
    if call.excinfo is not None and call.when == "call":
        ...
        error_id = error_logger.log_error(
            ...
            full_traceback=exc_traceback,
            file_path=file_path,
            line_number=line_number
        )

        # Attempt automatic resolution
        test_command = f"pytest {item.nodeid} -v"
        success, strategy = auto_resolver.resolve_and_retry(
            error_id=error_id,
            error_type=exc_type,
            error_message=exc_message,
        ...
        if success:
            error_logger.logger.info(
                f"✓ Test {item.nodeid} passed after auto-resolution"
            )
        elif strategy == "OPERATOR_ASSISTANCE_REQUESTED":
            error_logger.logger.critical(
                f"✗ Test {item.nodeid} requires operator assistance (exceeded {error_logger.MAX_AUTO_RESOLVE_ATTEMPTS} attempts)"
            )

def pytest_sessionfinish(session, exitstatus):
    """Generate error summary at end of test session."""
    summary = error_logger.generate_error_summary()

    print("\n" + "="*80)
    print("TEST ERROR SUMMARY")
    print("="*80)
    print(f"Total Errors: {summary['total_errors']}")
    print(f"Resolved Errors: {summary['resolved_errors']}")
    print(f"Pending Errors: {summary['pending_errors']}")
    print(f"Operator Assistance Requested: {summary['operator_assistance_requested']}")

    if summary['operator_assistance_requested'] > 0:
        print("\n⚠️  ATTENTION: Some errors require operator assistance")
        print(f"   See: {{folders.tests}}/artifacts/error-logs/operator-assistance-requests.jsonl")
...
Update `Makefile` to include error resolution:

```makefile
# Test with automatic error resolution
test-auto-resolve:
	@echo "Running tests with automatic error resolution..."
	pytest -v --tb=short || true
	@python -c "from error_handler import error_logger; summary = error_logger.generate_error_summary(); \
	             print(f'\n✓ Resolved: {summary[\"resolved_errors\"]} | ✗ Pending: {summary[\"pending_errors\"]} | ⚠ Assistance: {summary[\"operator_assistance_requested\"]}')"

# View error logs
test-errors:
	@echo "Recent test errors:"
	@tail -50 {{folders.tests}}/artifacts/error-logs/test-errors-*.log

# View operator assistance requests
test-assistance:
	@echo "Tests requiring operator assistance:"
	@cat {{folders.tests}}/artifacts/error-logs/operator-assistance-requests.jsonl | jq .
```

#### TypeScript/Node.js Error Handler

For TypeScript/Vitest projects, create `{{folders.tests}}/errorHandler.ts`:

```typescript
/**
 * Error logging and automatic resolution for TypeScript/Node.js tests
 * Generated by: ai-ley build-unit-tests | timestamp: 2025-11-10T00:00:00Z
 */
import fs from 'fs';
...
const ERROR_LOG_DIR = path.join(process.cwd(), 'tests', 'artifacts', 'error-logs');
const MAX_AUTO_RESOLVE_ATTEMPTS = 10;
...
interface TestError {
  ...
  operatorAssistanceRequested: boolean;
}

class TestErrorLogger {
  private errorRegistry: Map<string, TestError> = new Map();
  private resolutionLogPath: string;
  private errorSummaryPath: string;
  private assistanceRequestsPath: string;
  ...
  constructor() {
    ...
    this.resolutionLogPath = path.join(ERROR_LOG_DIR, 'resolution-history.jsonl');
    this.errorSummaryPath = path.join(ERROR_LOG_DIR, 'error-summary.json');
    this.assistanceRequestsPath = path.join(ERROR_LOG_DIR, 'operator-assistance-requests.jsonl');

    this.loadErrorHistory();
  }

  private loadErrorHistory(): void {
    if (fs.existsSync(this.resolutionLogPath)) {
      const lines = fs.readFileSync(this.resolutionLogPath, 'utf-8').split('\n');
      for (const line of lines) {
        if (line.trim()) {
          const error = JSON.parse(line) as TestError;
          this.errorRegistry.set(error.errorId, error);
        }
      }
    }
  }
  ...
  private generateErrorId(
    ...
    const signature = `${errorType}:${errorMessage}:${filePath}:${lineNumber}`;
  ...
  public logError(
    ...
    const errorId = this.generateErrorId(errorType, errorMessage, filePath, lineNumber);

    if (this.errorRegistry.has(errorId)) {
      const error = this.errorRegistry.get(errorId)!;
      error.resolutionAttempts++;
      ...
      this.errorRegistry.set(errorId, {
        ...
        resolved: false,
        operatorAssistanceRequested: false,
      });
    }

    const error = this.errorRegistry.get(errorId)!;
    console.error(`[ERROR ${errorId}] ${testName}: ${errorType} - ${errorMessage}`);
    console.error(`Location: ${filePath}:${lineNumber}`);
    console.error(`Attempt: ${error.resolutionAttempts}/${MAX_AUTO_RESOLVE_ATTEMPTS}`);

    this.appendToResolutionLog(error);
    return errorId;
  }

  private appendToResolutionLog(error: TestError): void {
    fs.appendFileSync(this.resolutionLogPath, JSON.stringify(error) + '\n');
  }

  public markResolved(errorId: string, resolutionStrategy: string): void {
    const error = this.errorRegistry.get(errorId);
    if (error) {
      error.resolved = true;
      error.resolutionStrategy = resolutionStrategy;
      console.log(`✓ Error [${errorId}] resolved using: ${resolutionStrategy}`);
      this.appendToResolutionLog(error);
    }
  }

  public shouldRequestOperatorAssistance(errorId: string): boolean {
    const error = this.errorRegistry.get(errorId);
    return error ? error.resolutionAttempts >= MAX_AUTO_RESOLVE_ATTEMPTS && !error.resolved : false;
  }

  public requestOperatorAssistance(errorId: string): object {
    const error = this.errorRegistry.get(errorId);
    if (!error) return { error: 'Unknown error ID' };

    error.operatorAssistanceRequested = true;
    ...
    const assistanceRequest = {
      ...
      traceback: error.fullTraceback,
      timestamp: new Date().toISOString(),
      message: `OPERATOR ASSISTANCE REQUIRED: Test '${error.testName}' failed ${error.resolutionAttempts} times. Manual intervention needed.`,
    };

    console.error('\n⚠️  ' + assistanceRequest.message);
    fs.appendFileSync(this.assistanceRequestsPath, JSON.stringify(assistanceRequest) + '\n');
    this.appendToResolutionLog(error);

    return assistanceRequest;
  }

  public generateErrorSummary(): object {
    const totalErrors = this.errorRegistry.size;
    const resolvedErrors = Array.from(this.errorRegistry.values()).filter((e) => e.resolved).length;
    const pendingErrors = totalErrors - resolvedErrors;
    const assistanceRequested = Array.from(this.errorRegistry.values()).filter(
      (e) => e.operatorAssistanceRequested,
    ).length;
    ...
    const summary = {
      ...
      resolvedErrors,
      pendingErrors,
      operatorAssistanceRequested: assistanceRequested,
      errorsByType: {} as Record<string, number>,
      errorsByTest: {} as Record<string, number>,
      resolutionStrategies: {} as Record<string, number>,
    };

    for (const error of this.errorRegistry.values()) {
      summary.errorsByType[error.errorType] = (summary.errorsByType[error.errorType] || 0) + 1;
      summary.errorsByTest[error.testName] = (summary.errorsByTest[error.testName] || 0) + 1;
      if (error.resolutionStrategy) {
        summary.resolutionStrategies[error.resolutionStrategy] =
          (summary.resolutionStrategies[error.resolutionStrategy] || 0) + 1;
      }
    }

    fs.writeFileSync(this.errorSummaryPath, JSON.stringify(summary, null, 2));
    return summary;
  }
}

class AutoResolver {
  private logger: TestErrorLogger;

  constructor(logger: TestErrorLogger) {
    this.logger = logger;
  }

  public async resolveAndRetry(
    errorId: string,
    errorType: string,
    errorMessage: string,
    testCommand: string,
  ): Promise<[boolean, string | null]> {
    if (this.logger.shouldRequestOperatorAssistance(errorId)) {
      this.logger.requestOperatorAssistance(errorId);
      return [false, 'OPERATOR_ASSISTANCE_REQUESTED'];
    }

    const strategy = await this.getResolutionStrategy(errorType, errorMessage);

    if (strategy) {
      console.log(`Attempting resolution for [${errorId}] using: ${strategy}`);

      try {
        execSync(testCommand, { stdio: 'inherit' });
        this.logger.markResolved(errorId, strategy);
        return [true, strategy];
      } catch (error) {
        console.warn(`Resolution strategy '${strategy}' did not fix error [${errorId}]`);
        return [false, null];
      }
    }
  ...
  private async getResolutionStrategy(
    errorType: string,
    errorMessage: string,
  ): Promise<string | null> {
    switch (errorType) {
      case 'ModuleNotFoundError':
      case 'Error: Cannot find module':
        return this.resolveModuleNotFound(errorMessage);
      case 'TimeoutError':
        return 'timeout_increase';
      case 'ENOENT':
        return this.resolveFileNotFound(errorMessage);
      default:
        return null;
    }
  }

  private resolveModuleNotFound(errorMessage: string): string | null {
    const match = errorMessage.match(/Cannot find module '([^']+)'/);
    if (match) {
      const moduleName = match[1];
      try {
        execSync(`npm install ${moduleName}`, { stdio: 'inherit' });
        return `npm_install:${moduleName}`;
      } catch (error) {
  ...
  private resolveFileNotFound(errorMessage: string): string | null {
    const match = errorMessage.match(/ENOENT: no such file or directory, .* '([^']+)'/);
    if (match) {
...
export const autoResolver = new AutoResolver(errorLogger);
```

#### Vitest Integration

Update `vitest.config.ts` to include error handling:

```typescript
import { defineConfig } from 'vitest/config';
import { errorLogger, autoResolver } from './tests/errorHandler';
...
export default defineConfig({
  test: {
    ...
    onConsoleLog: (log, type) => {
      ...
      if (type === 'error') {
        // Error logging is handled by test hooks
      }
...
afterEach((context) => {
  // Log test errors automatically
  if (context.task.result?.state === 'fail' && context.task.result?.error) {
    ...
    errorLogger.logError(
      ...
      error.message,
      error.stack || '',
      context.task.file?.filepath || 'unknown',
      0, // Line number extraction would require stack parsing
    );
  }
});
...
afterAll(() => {
  ...
  const summary = errorLogger.generateErrorSummary();
  console.log('\n' + '='.repeat(80));
  console.log('TEST ERROR SUMMARY');
  console.log('='.repeat(80));
  console.log(`Total Errors: ${(summary as any).totalErrors}`);
  console.log(`Resolved Errors: ${(summary as any).resolvedErrors}`);
  console.log(`Pending Errors: ${(summary as any).pendingErrors}`);
  console.log(`Operator Assistance Requested: ${(summary as any).operatorAssistanceRequested}`);

  if ((summary as any).operatorAssistanceRequested > 0) {
    console.log('\n⚠️  ATTENTION: Some errors require operator assistance');
    console.log('   See: tests/artifacts/error-logs/operator-assistance-requests.jsonl');
  }
  console.log('='.repeat(80) + '\n');
});
```
...
├── conftest.py                    # Pytest configuration with auto-resolution hooks
├── __init__.py
├── timeout_watchdog.py            # Timeout enforcement
├── retry_decorator.py             # Flaky test retries
├── error_handler.py               # Error logging and tracking system
├── auto_resolver.py               # Automatic error resolution engine
...
└── artifacts/                     # Test artifacts
    ...
    ├── profiles/                  # Performance profiles
    └── error-logs/                # Error logs and resolution history
        ├── test-errors-YYYYMMDD.log          # Daily error logs
        ├── resolution-history.jsonl          # All error resolutions
        ├── error-summary.json                # Error summary statistics
        └── operator-assistance-requests.jsonl # Errors requiring manual intervention
```
...
This prompt implements all testing requirements from `agent guidelines`:
...
4. **Coverage Thresholds**: Enforced at 80% minimum; builds fail if coverage drops below threshold
...
11. **Error Logging & Auto-Resolution**: Comprehensive error tracking with automatic resolution attempts and operator assistance escalation
    - All test errors logged to `{{folders.tests}}/artifacts/error-logs/` with unique error IDs
    - Automatic resolution attempts for common error types (ImportError, ModuleNotFoundError, etc.)
    - Maximum 10 auto-resolution attempts per unique error before requesting operator assistance
    - Resolution history tracked in JSONL format for analysis and learning
    - Operator assistance requests logged separately for manual intervention
    - Error summary generated after each test session with resolution statistics

### Error Resolution Workflow

1. **Test Failure Detection**: When a test fails, error details are captured and logged with unique error ID
2. **Auto-Resolution Attempt**: System attempts to resolve using appropriate strategy:
   - **ImportError/ModuleNotFoundError**: Auto-install missing packages via pip
   - **FileNotFoundError**: Create missing directories for test fixtures
   - **TimeoutError**: Flag for timeout threshold review
   - **ConnectionError**: Check service availability and attempt restart
   - **Other errors**: Log for manual review
3. **Retry Execution**: Test is automatically re-run after resolution attempt
4. **Success Tracking**: Successful resolutions are logged with strategy used
5. **Escalation**: After 10 failed resolution attempts for the same error:
   - System requests operator assistance
   - Detailed error report generated in `operator-assistance-requests.jsonl`
   - Critical log entry created for immediate attention
   - Test continues to prevent blocking entire suite
6. **Session Summary**: At end of test run, comprehensive summary shows:
   - Total errors encountered
   - Successfully resolved errors
   - Pending errors
   - Errors requiring operator assistance
...
- **Review error logs regularly** to identify patterns and systemic issues
- **Address operator assistance requests promptly** to prevent blocking development
- **Use error resolution history** to improve test suite and code quality
- **Monitor auto-resolution success rates** to refine resolution strategies
...
- Check error logs in `{{folders.tests}}/artifacts/error-logs/test-errors-YYYYMMDD.log`
- Verify error type is supported in `auto_resolver.py`
- Review resolution strategies and add custom resolvers as needed
- Check that dependencies can be installed (network access, package availability)

**Too many operator assistance requests**:

- Review common error patterns in `operator-assistance-requests.jsonl`
- Identify root causes and fix in code or test configuration
- Add custom resolution strategies for project-specific errors
- Consider increasing MAX_AUTO_RESOLVE_ATTEMPTS if errors are transient

**Error logging consuming too much disk space**:

- Implement log rotation in error_handler.py
- Archive old error logs periodically
- Set retention policies for resolved errors
