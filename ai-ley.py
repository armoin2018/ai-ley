...
class TemplateProvisioner:
    def __init__(self):
        ...
        self.templates_dir = self.base_dir / ".ai-ley"
        self.common_dir = self.base_dir / ".ai-ley" / "common"
        self.provision_map = self.base_dir / "ai-ley.map.yaml"
        self.active_ai_tools = set()  # Track currently provisioned AI tools

    def load_git_repos_config(self):
        """Load git repositories config from ai-ley.map.yaml under 'git_repos'"""
        provision_config = self.load_provision_map_config()
        if not provision_config or 'git_repos' not in provision_config:
            raise FileNotFoundError("Missing 'git_repos' key in ai-ley.map.yaml")
        return provision_config['git_repos']
    
    def load_provision_map_config(self):
        """Load provision mapping configuration from ai-ley.map.yaml"""
        if not self.provision_map.exists():
            raise FileNotFoundError(f"Configuration file not found: {self.provision_map}")
    ...
    def provision_project_type(self, project_type, variables):
        """Provision a project type using doc_handler configuration from ai-ley.map.yaml"""
        provision_config = self.load_provision_map_config()
        
        if 'doc_handler' not in provision_config:
            raise ValueError("No doc_handler configuration found in ai-ley.map.yaml")
            
        doc_handler = provision_config['doc_handler']
        
        if project_type not in doc_handler:
            raise ValueError(f"Unknown project type: {project_type}")
            
        project_config = doc_handler[project_type]
        
        print(f"Provisioning project type '{project_type}'...")
        
        # Get default variables from template_vars
        default_variables = provision_config.get('template_vars', {})
        default_variables.update(variables)
        
        # Process file mappings
        for file_config in project_config.get('files', []):
    ...
    def _provision_ai_tool_new(self, ai_tool, provision_config, concurrent_mode=False, git_repos=False, no_overwrite=False):
        """Provision AI tool using new ai-ley.map.yaml format"""
    ...
    def _copy_prompts_for_ai_tool(self, ai_tool):
        ...
        # Check for prompts in both .ai-ley/common/prompts and docs/prompts
        prompts_sources = [
    ...
    def list_available_project_types(self):
        """List all available project types from doc_handler in ai-ley.map.yaml"""
        provision_config = self.load_provision_map_config()
        return provision_config.get('doc_handler', {})
    
    def process_file_with_variables(self, source_path, target_path, variables):
        """Process a file and substitute variables"""
        if self._is_text_file(source_path, {'text_extensions': ['.md', '.yml', '.yaml', '.json', '.js', '.ts', '.py']}):
    ...
    def clean_provisioned_files(self, ai_tool):
        ...
        for copy_instruction in tool_config.get('copies', []):
    ...
    def pull_git_repo(self, repo_key, target_dir):
        """Clone or update git repo and copy mapped folders into target_dir/subfolder using git_repos from ai-ley.map.yaml"""
        git_repos = self.load_git_repos_config()
        if repo_key not in git_repos:
            raise ValueError(f"Unknown repo key: {repo_key}")
        repo_cfg = git_repos[repo_key]
        repo_url = repo_cfg['url']
        subfolder = repo_cfg['subfolder']
        folders = repo_cfg['folders']
        branch = repo_cfg.get('branch', 'main')

        # Use .ai-ley/external as the cache directory for all downloaded git repos
        cache_dir = self.base_dir / '.ai-ley' / 'external'
        ...
        if repo_local.exists():
            ...
            os.system(f"cd {repo_local} && git fetch && git reset --hard origin/{branch}")
        else:
            print(f"Cloning repo {repo_key} from {repo_url}...")
            os.system(f"git clone -b {branch} {repo_url} {repo_local}")
        ...
        print(f"Git resources from {repo_key} pulled into {target_base}")
    ...
    def _process_mapping(self, mapping, target_base, settings):
        """Process a single mapping from ai-ley.map.yaml"""
...
def main():
    ...
    # Pull git repo resources command
    pull_repo_parser = subparsers.add_parser('pull-repo', help='Pull resources from git repo')
    pull_repo_parser.add_argument('--repo', required=True, help='Repo key from git_repos in ai-ley.map.yaml')
    ...
    try:
        if args.command == 'list':
            ...
            provisioner.pull_git_repo(args.repo, args.target_dir)
